...
import AdSense from "@/src/components/Elements/AdSense";
  ...
  if (blog.image) {
    imageList =
      typeof blog.image.src === "string"
        ? [blog.image.src.startsWith('/') ? siteMetadata.siteUrl + blog.image.src : blog.image.src]
        : blog.image;
  }
  const ogImages = imageList.map((img) => {
    try {
      const imageUrl = img.includes("http") ? img : siteMetadata.siteUrl + img;
      new URL(imageUrl); // Validate URL
      return { url: imageUrl };
    } catch (error) {
      console.warn(`Invalid image URL: ${img}, using fallback`);
      return { url: siteMetadata.siteUrl + siteMetadata.socialBanner };
    }
  });
...
export default async function BlogPage({ params }) {
  ...
  return (
    ...
    <script
      ...
      {/* Ad after blog header */}
      <div className="my-8 px-5 md:px-10">
        <AdSense />
      </div>
